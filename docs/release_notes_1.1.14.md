# codex_workflow 1.1.14 — Experimental release notes
...
Lifecycle guides and metadata now live under `codex_workflow/operate/`:
`bootstrap.md`, `check_update.md`, `disable.md`, `enable.md`, `install.md`,
`personalization_guide.md`, `remove.md`, `update.md`, `VERSION`, and
`user_AGENTS.md`.

The executable entry point is `codex_workflow/runtime/workflow.py`. Command
...
```sh
python3 <extracted>/codex_workflow/runtime/workflow.py update \
  --source <extracted>/codex_workflow --project <project> --json
```
