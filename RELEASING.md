...
`VERSION=1.2.0` and tag `v1.2.0`. GitHub's prerelease flag is independent of
the SemVer string; the initial releases are marked as prereleases by the
workflow.
The command examples below use the current package version, `1.2.0`; replace
...
python3 scripts/package_release.py --release-tag v1.2.0 --output-dir dist
python3 scripts/package_release.py --verify dist/codex_workflow-1.2.0.zip --version 1.2.0
```
...
py -3.11 scripts/package_release.py --release-tag v1.2.0 --output-dir dist
py -3.11 scripts/package_release.py --verify dist\codex_workflow-1.2.0.zip --version 1.2.0
```
...
```sh
git status --short
git tag -a v1.2.0 -m "codex_workflow v1.2.0"
...
gh release create v1.2.0 \
  dist/codex_workflow-1.2.0.zip \
  dist/SHA256SUMS \
  --title "codex_workflow v1.2.0" \
  --generate-notes \
  --prerelease
```
