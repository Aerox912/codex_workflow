...
`VERSION=1.1.7` and tag `v1.1.7`. GitHub's prerelease flag is independent of
the SemVer string; the initial releases are marked as prereleases by the
workflow.
The command examples below use the current package version, `1.1.7`; replace
...
python3 scripts/package_release.py --release-tag v1.1.7 --output-dir dist
...
py -3.11 scripts/package_release.py --release-tag v1.1.7 --output-dir dist
py -3.11 scripts/package_release.py --verify dist\codex_workflow-1.1.7.zip
```
...
```sh
git status --short
git tag -a v1.1.7 -m "codex_workflow v1.1.7"
...
gh release create v1.1.7 \
  dist/codex_workflow-1.1.7.zip \
  dist/SHA256SUMS \
  --title "codex_workflow v1.1.7" \
  --generate-notes \
  --prerelease
```
