...
Delegate endpoint state, non-browser uploads, external search,
routine Git/status collation, tool or API discovery, logs, environment checks,
and operational diagnostics. The main task exclusively owns visible browser
control, screenshots, confirmations, tab identity, and live ChatGPT submissions
and reads. Workers may prepare prompts and analyze returned evidence only.
If a decisive check genuinely cannot be delegated,
