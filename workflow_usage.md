...
```text
Download and extract the latest `codex_workflow-<version>.zip` asset (not GitHub's Source code archive) from https://github.com/Aerox912/codex_workflow/releases. Verify it against `SHA256SUMS`, then read the bundled `codex_workflow/bootstrap.md` and follow it exactly.
```
...
- **Heavy route** — an Astra, Sol, or Terra main agent with subagent support orchestrates fixed
